...
func (d *DaemonServer) abortHandoff(reason string) {
	...
	_ = d.sendWS(map[string]any{
		"type": "update_failed", "hostId": d.configNow().HostID, "reason": reason,
	})
}
...
func gatewayBaseURL(d *DaemonServer) string {
	...
	if d.configNow() == nil || d.configNow().GatewayURL == "" {
		return ""
	}
	u, err := url.Parse(d.configNow().GatewayURL)
	if err != nil || u.Host == "" {
		return ""
	}
	scheme := "http"
...
func (d *DaemonServer) debugMirror(raw []byte) {
	...
	info["configGateway"] = d.configNow().GatewayURL
	st := d.updateChecker()
	st.mu.Lock()
