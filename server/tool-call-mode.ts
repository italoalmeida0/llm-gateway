 ...
 * server/proxy/markup-tools.ts) and, for the worst offenders, actively move
 * the tool schema into an in-band instruction instead of the native `tools`
 * array.
 *
 ...
 *     markup recovery is disabled (DSML stays — it is a separate, always-on
 *     DeepSeek safety net).
 ...
 *     (xiaomi XML, MiniMax `<invoke>`, Hermes/Qwen JSON). Use it when a model
 *     sometimes leaks markup even with native calling on.
 *   - `workaround`: ACTIVE. Native `tools` are stripped and the schema is
 *     re-delivered as an in-band instruction (teaching the supported
 *     formats); the response is recovered from every dialect. Auto-selected
 *     for models whose id contains "xiaomi".
 *
...
export const TOOL_CALL_MODES: readonly ToolCallMode[] = ["native", "fallback", "workaround"];

export function normalizeToolCallMode(v: unknown): ToolCallMode {
  return v === "fallback" || v === "workaround" ? v : "native";
}
...
/** True when the mode also strips native tools and injects the instruction. */
export function isWorkaroundMode(mode: ToolCallMode): boolean {
  return mode === "workaround";
}
