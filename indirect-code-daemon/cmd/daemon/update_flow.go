...
func runUpdateStart(dataDir, cfgPath string, p updateStartParams) int {
	...
	server := newUpdateHost(dataDir, cfgPath)
	server.sharedDir = filepath.Join(server.rootDir(), "external")
	if err := server.loadConfig(); err != nil || server.configNow() == nil {
		logf("config: %v", err)
		return 1
	}
	...
	// default install, and ambient mirror settings may belong to another
	// host. An explicit INDIRECT_REPO_RAW override still WINS (it is more
	// specific than the gateway default — the harness mirrors and the
	// gateway-death e2e rely on it; os/exec dedups env keys last-wins, so
	// appending INDIRECT_GATEWAY unconditionally would shadow it).
	if os.Getenv("INDIRECT_REPO_RAW") == "" {
...
func updateStartFail(server *DaemonServer, updaterProc *os.Process, p updateStartParams, reason string, logf func(string, ...any)) int {
	...
	_ = server.sendWS(map[string]any{
		"type": "update_failed", "hostId": server.configNow().HostID, "reason": reason,
	})
...
func doUpdateEndPromote(d *DaemonServer) error {
	...
	_ = d.sendWS(map[string]any{
		"type": "update_done", "hostId": d.configNow().HostID, "version": daemonVersion,
	})
	st := d.updateChecker()
	st.mu.Lock()
	st.available = ""
