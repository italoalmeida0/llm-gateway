...
func TestHandoffPauseResumesRunningTurn(t *testing.T) {
	...
	if err := storeOf(d).saveSessionSync(rec); err != nil {
		t.Fatal(err)
	}
	ww, err := storeOf(d).openWAL(rec.ID, &walHeader{TurnIndex: 2, StartedAt: 42, Prompt: "go"})
	if err != nil {
		t.Fatal(err)
	}
	...
	fused, h, err := storeOf(d2).loadSessionFused("run1")
	...
	if hdr, w := turnResumeCandidate(fused, storeOf(d2), "run1"); hdr == nil {
...
func TestCopyToSlotCopiesLiveStateAsIs(t *testing.T) {
	...
	ww, err := storeOf(d).openWAL(rec.ID, &walHeader{TurnIndex: 1, StartedAt: 1, Prompt: "x"})
	if err != nil {
		t.Fatal(err)
	}
