...
<div class="rounded-xl border border-line bg-elev/40 p-4 sm:p-5 space-y-4">
  <h3 id="sec-mcp" class="text-sm font-semibold text-ink-100 flex items-center gap-2 scroll-mt-2">
    ...
    <span class="px-1.5 py-0.2 rounded-full bg-ink-800 text-[10px] text-ink-400">{Object.keys(ctx.mcpServers()).length}</span>
  </h3>
  <div class="space-y-4 text-xs">
    <div class="border border-line rounded-xl p-3 bg-ink-900/50 space-y-2">
      ...
      <div class="grid grid-cols-2 gap-2">
        <input
          ...
          value={ctx.newMcpName()}
          onInput={(e) => ctx.setNewMcpName(e.currentTarget.value)}
        />
        <input
          type="text"
          ...
          class="bg-ink-900 border border-line rounded-lg px-2.5 py-1.5 text-ink-100"
          value={ctx.newMcpCmd()}
          onInput={(e) => ctx.setNewMcpCmd(e.currentTarget.value)}
        />
      </div>
      ...
      <input
        ...
        class="w-full bg-ink-900 border border-line rounded-lg px-2.5 py-1.5 text-ink-100"
        value={ctx.newMcpArgs()}
        onInput={(e) => ctx.setNewMcpArgs(e.currentTarget.value)}
      />
      <div class="flex justify-end">
        <button
          onClick={ctx.handleAddMcpServer}
    ...
    <div class="space-y-2">
      <div class="font-semibold text-ink-300">
        Active MCP Servers ({Object.keys(ctx.mcpServers()).length})
      </div>
      <For
        each={Object.entries(ctx.mcpServers())}
        fallback={
      ...
      >
        {([name, srv]) => (
          <div class="p-3 rounded-xl border border-line bg-ink-900 flex items-center justify-between">
            ...
            <button
              onClick={() => ctx.handleDeleteMcpServer(name)}
