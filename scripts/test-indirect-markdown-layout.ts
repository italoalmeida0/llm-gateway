...
try {
  for (const width of [320, 360, 390, 768, 1024, 1440, 1920]) for (const theme of ["light", "dark"]) {
    ...
    assert.equal(await page.locator('#markdown .katex').count(), 1);
    ...
    assert(geometry.inside && geometry.width >= (touch ? 44 : 32) && geometry.height >= (touch ? 44 : 32));
    ...
    const download = page.getByRole('button', { name: 'Download table', exact: true });
    if (touch) await download.tap(); else await download.click();
    const menu = page.getByRole('group', { name: 'Table downloads' });
    await page.waitForTimeout(100);
    assert(await menu.evaluate((el: HTMLElement) => { const r = el.getBoundingClientRect(); return r.left >= 0 && r.right <= innerWidth && r.top >= 0 && r.bottom <= innerHeight; }));
    ...
    if (touch) {
      for (const actions of await page.locator('.rc-message-actions').all()) {
        ...
        assert(await actions.locator('button').first().evaluate((el: HTMLElement) => el.offsetHeight >= 44));
      }
    }
